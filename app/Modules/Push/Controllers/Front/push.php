    ...
    if ($result) {
        ...
        for ($m = 0; $m < $ibid; $m++) {
            ...
            if (ctrl_aff($ihome, $catid)) {
                ...
                echo "document.write('&nbsp;-&nbsp;<a href=javascript:onclick=register(\"App-push\",\"op=new_show&sid=$sid&offset=$m\"); style=\"font-size: 11px;\">" . htmlspecialchars(aff_langue($title), ENT_COMPAT | ENT_HTML401, cur_charset) . "</a><br />');\n";
            }
        }
    }
...
function new_show($sid, $offset)
{
    ...
    if ($result) {
        ...
        $title = str_replace("'", "\'", $title);
        echo "document.write('<span style=\"font-size: 11px;\"><b>.:|<a href=\"Config::get('npds.nuke_url')/article.php?sid=$sid\" target=\"_blank\">" . aff_langue($title) . "</a>|:.</b></span><br />');\n";
...
function push_faq()
{
    ...
    while (list($id_cat, $categories) = sql_fetch_row($result)) {
        ...
        echo "document.write('&nbsp;-&nbsp;<a href=javascript:onclick=register(\"App-push\",\"op=faq_show&id_cat=$id_cat\"); style=\"font-size: 11px;\">" . aff_langue($categories) . "</a><br />');\n";
    }
...
function push_links()
{
    ...
    while (list($cid, $title, $cdescription) = sql_fetch_row($result)) {
        ...
        echo "document.write('<td width=\"49%\" valign=\"top\"><a href=javascript:onclick=register(\"App-push\",\"op=viewlink&cid=$cid\"); style=\"font-size: 11px;\"><b>" . aff_langue($title) . "</b></a> ($cnumrows)');\n";
        ...
        while (list($sid, $stitle) = sql_fetch_row($result2)) {
            ...
            $title = str_replace("'", "\'", $title);
            echo "document.write('&nbsp;<a href=javascript:onclick=register(\"App-push\",\"op=viewslink&sid=$sid\"); style=\"font-size: 11px;\">" . aff_langue($stitle) . "</a>');\n";
            $space++;
        }
...
function viewlink_show($cid, $min)
{
    ...
    if ($numrows != 0) {
        ...
        while (list($sid, $title) = sql_fetch_row($subresult)) {
            ...
            $title = str_replace("'", "\'", $title);
            echo "document.write('<li><a href=javascript:onclick=register(\"App-push\",\"op=viewslink&sid=$sid\"); style=\"font-size: 11px;\">" . aff_langue($title) . "</a> ($numrows)</li>');\n";
        }
    ...
    while (list($lid, $title) = sql_fetch_row($result)) {
        ...
        echo "document.write('<li><a href=\"Config::get('npds.nuke_url')/links.php?op=visit&amp;lid=$lid\" target=\"_blank\" style=\"font-size: 11px;\">" . aff_langue($title) . "</a></li><br />');\n";
    }

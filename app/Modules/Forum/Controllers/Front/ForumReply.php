 ...
 * Undocumented class
 */
class ForumReply extends FrontController
{
        ...
        if ($submitS) {
            ...
            if ($stop != 1) {
                ...
                if (($m['topic_notify'] == 1) && ($m['uname'] != $userdata['uname'])) {
                    ...
                    $message .= "<a href=\"Config::get('npds.nuke_url')/viewtopic.php?topic=$topic&forum=$forum&start=9999#lastpost\">Config::get('npds.nuke_url')/viewtopic.php?topic=$topic&forum=$forum&start=9999</a>\n\n";
                ...
                echo '
                <h4 class="my-3">' . __d('forum', 'Poster une réponse dans le sujet') . '</h4>
                <p class="alert alert-danger">' . __d('forum', 'Vous devez taper un message à poster.') . '</p>
                <a class="btn btn-outline-primary" href="javascript:history.go(-1)" >' . __d('forum', 'Retour en arrière') . '</a>';
            }
        } else {
            include('header.php');
            ...
            <h4 class="d-none d-sm-block my-3"><img width="48" height="48" class=" rounded-circle me-3" src="' . $imgava . '" alt="" />' . __d('forum', 'Poster une réponse dans le sujet') . '</h4>
            <form action="reply.php" method="post" name="coolsus">';
        
            echo '<blockquote class="blockquote d-none d-sm-block"><p>' . __d('forum', 'A propos des messages publiés :') . '<br />';
            ...
            if ($allow_to_reply) {
                ...
                if ($citation && !$submitP) {
                    ...
                    if ($r = sql_query($sql)) {
                        ...
                        $reply = ($m['post_time'] != '' && $m['uname'] != '') ?
                            '<blockquote class="blockquote">' . __d('forum', 'Citation') . ' : <strong>' . $m['uname'] . '</strong><br />' . $text . '</blockquote>' :
                            $text . "\n";
                    ...
                    if ($allow_sig == 1 || $sig == 'on') {
                        ...
                        if (($forum_type != '6') and ($forum_type != '5')) {
                            ...
                            <div class="form-check">
                                ...
                                <small class="help-block">' . __d('forum', 'Cela peut être retiré ou ajouté dans vos paramètres personnels') . '</small>
                            </div>
                            </div>';
                        }
                    }
                ...
                <div class="mb-3 row">
                    <div class="col-sm-12">
                        ...
                        <button class="btn btn-primary" type="submit" value="' . __d('forum', 'Valider') . '" name="submitS" accesskey="s" title="' . __d('forum', 'Valider') . '" data-bs-toggle="tooltip" >' . __d('forum', 'Valider') . '</button>&nbsp;
                ...
                echo '<div class="alert alert-danger">' . __d('forum', 'Vous n\'êtes pas autorisé à participer à ce forum') . '</div>';
        
            echo '</form>';
        
            if ($allow_to_reply) {
                echo '<h4 class="my-3">' . __d('forum', 'Aperçu des sujets :') . '</h4>';
        
                $post_aff = $Mmod ? '' : " AND post_aff='1' ";
                $sql = "SELECT * FROM posts WHERE topic_id='$topic' AND forum_id='$forum'" . $post_aff . "ORDER BY post_id DESC limit 0,10";
                ...
                do {
                    ...
                    if ($myrow['poster_id'] !== '0') {
                        ...
                        if ($user or autorisation(-127)) {
                            if ($posterdata['uid'] != 1 and $posterdata['uid'] != '')
                                $useroutils .= '<a class="list-group-item text-primary text-center text-md-start" href="user.php?op=userinfo&amp;uname=' . $posterdata['uname'] . '" target="_blank" title="' . __d('forum', 'Profil') . '" data-bs-toggle="tooltip"><i class="fa fa-user fa-2x align-middle fa-fw"></i><span class="ms-3 d-none d-md-inline">' . __d('forum', 'Profil') . '</span></a>';
                            ...
                            if ($posterdata['femail'] != '')
                                $useroutils .= '<a class="list-group-item text-primary text-center text-md-start" href="mailto:' . anti_spam($posterdata['femail'], 1) . '" target="_blank" title="' . __d('forum', 'Email') . '" data-bs-toggle="tooltip"><i class="fa fa-at fa-2x align-middle fa-fw"></i><span class="ms-3 d-none d-md-inline">' . __d('forum', 'Email') . '</span></a>';
                            
                            if ($myrow['poster_id'] != 1 and array_key_exists($ch_lat, $posterdata_extend)) {
                                if ($posterdata_extend[$ch_lat] != '')
                                    $useroutils .= '<a class="list-group-item text-primary text-center text-md-start" href="modules.php?ModPath=geoloc&amp;ModStart=geoloc&amp;op=u' . $posterdata['uid'] . '" title="' . __d('forum', 'Localisation') . '" ><i class="fas fa-map-marker-alt fa-2x align-middle fa-fw">&nbsp;</i><span class="ms-3 d-none d-md-inline">' . __d('forum', 'Localisation') . '</span></a>';
                            }
                        }
        
                        if ($posterdata['url'] != '')
                            $useroutils .= '<a class="list-group-item text-primary text-center text-md-start" href="' . $posterdata['url'] . '" target="_blank" title="' . __d('forum', 'Visiter ce site web') . '" data-bs-toggle="tooltip"><i class="fas fa-external-link-alt fa-2x align-middle fa-fw"></i><span class="ms-3 d-none d-md-inline">' . __d('forum', 'Visiter ce site web') . '</span></a>';
                        
                        if ($posterdata['mns'])
                            $useroutils .= '<a class="list-group-item text-primary text-center text-md-start" href="minisite.php?op=' . $posterdata['uname'] . '" target="_blank" target="_blank" title="' . __d('forum', 'Visitez le minisite') . '" data-bs-toggle="tooltip"><i class="fa fa-2x fa-desktop align-middle fa-fw"></i><span class="ms-3 d-none d-md-inline">' . __d('forum', 'Visitez le minisite') . '</span></a>';
                    }
                    ...
                    echo '
                        ...
                        <div class="card-body">
                            <span class="text-muted float-end small" style="margin-top:-1rem;">' . __d('forum', 'Posté : ') . convertdate($myrow['post_time']) . '</span>

    ...
    public function index()
    {
        ...
        if (isset($user)) {
            ...
            if ($submitS) {
                ...
                if (strstr($to_user, ',')) {
                    ...
                    foreach ($tempo as $to_user) {
                        $res = sql_query("SELECT uid, user_langue FROM users WHERE uname='$to_user'");
            ...
            if ($delete_messages and $msg_id) {
                ...
                foreach ($msg_id as $v) {
                    if ($type == 'outbox')
                        $sql = "DELETE FROM priv_msgs WHERE msg_id='" . $v . "' AND from_userid='" . $userdata['uid'] . "' AND type_msg='1'";
                    else
                        $sql = "DELETE FROM priv_msgs WHERE msg_id='" . $v . "' AND to_userid='" . $userdata['uid'] . "'";
            ...
            if (isset($delete)) {
                if (isset($type) and $type == 'outbox')
                    $sql = "DELETE FROM priv_msgs WHERE msg_id='$msg_id' AND from_userid='" . $userdata['uid'] . "' AND type_msg='1'";
                else
            ...
            if ($classement) {
                ...
                $sql = "UPDATE priv_msgs SET dossier='$dossier' WHERE msg_id='$msg_id' AND to_userid='" . $userdata['uid'] . "'";
            ...
            if ($full_interface == 'short') {
                ...
                echo import_css($tmp_theme, Config::get('npds.language'), '', '', '');
                echo '
                </head>
            ...
            if ($reply || $send || $to_user) {
                ...
                if ($reply) {
                    $sql = "SELECT msg_image, subject, from_userid, to_userid FROM priv_msgs WHERE to_userid='" . $userdata['uid'] . "' AND msg_id='$msg_id' AND type_msg='0'";
                    $result = sql_query($sql);
                ...
                echo '
                <h2><a href="viewpmsg.php"><i class="me-2 fa fa-inbox"></i></a>' . __d('messenger', 'Message personnel') . '</h2>
                <hr />
                <blockquote class="blockquote">' . __d('messenger', 'A propos des messages publiés :') . '<br />' .
                        __d('messenger', 'Tous les utilisateurs enregistrés peuvent poster des messages privés.') . '</blockquote>';
                ...
                if ($submitP) {
                    echo '
                    <hr />
                    <h3>' . __d('messenger', 'Prévisualiser') . '</h3>
                ...
                echo '
                    ...
                    <div class="mb-3 row">
                        <label class="col-form-label col-sm-3" for="to_user">' . __d('messenger', 'Destinataire') . '</label>
                        <div class="col-sm-9">';
                ...
                if (!$reply) {
                    ...
                    echo $carnet . '<span class="small">' . __d('messenger', 'Carnet d\'adresses') . '</span></a>';
                }
                ...
                if (Config::get('npds.smilies')) {
                    echo '
                    <div class="mb-3 row">
                        <label class="col-form-label col-sm-12">' . __d('messenger', 'Icone du message') . '</label>
                        <div class="col-sm-12">
                ...
                if ($allow_html == 1)
                    echo '<span class="text-success float-end" title="HTML ' . __d('messenger', 'Activé') . '" data-bs-toggle="tooltip"><i class="fa fa-code fa-lg"></i></span>' . HTML_Add();
                else
                    echo '<span class="text-danger float-end" title="HTML ' . __d('messenger', 'Désactivé') . '" data-bs-toggle="tooltip"><i class="fa fa-code fa-lg"></i></span>';
                ...
                if ($reply and $message == '') {
                    ...
                    if ($result = sql_query($sql)) {
                        ...
                        if ($row['msg_time'] != '' && $row['uname'] != '')
                            $Xreply = $row['msg_time'] . ', ' . $row['uname'] . ' ' . __d('messenger', 'a écrit :') . "\n$text\n";
                        else
                            $Xreply = $text;
                    ...
                    ' . $Xreply . '
                        ...
                        $Xreply = __d('messenger', 'Pas de connexion à la base forums.') . "\n";
                ...
                echo '
                    ...
                    <span class="help-block text-end">
                        <button class="btn btn-outline-danger btn-sm" type="reset" value="' . __d('messenger', 'Annuler') . '" title="' . __d('messenger', 'Annuler') . '" data-bs-toggle="tooltip" ><i class="fas fa-times " ></i></button>
                        <button class="btn btn-outline-primary btn-sm" type="submit" value="' . __d('messenger', 'Prévisualiser') . '" name="submitP" title="' . __d('messenger', 'Prévisualiser') . '" data-bs-toggle="tooltip" ><i class="fa fa-eye "></i></button>
                    </span>
                        </div>
                ...
                <div class="mb-3 row">
                    <label class="col-form-label col-sm-3">' . __d('messenger', 'Options') . '</label>';
                ...
                if ($allow_html == 1) {
                    ...
                    <div class="col-sm-9 my-2">
                        ...
                        <label class="form-check-label" for="html">' . __d('messenger', 'Désactiver le html pour cet envoi') . '</label>
                        </div>';
                }
                ...
                if ($usermore['attachsig'] == '1') {
                    ...
                    <label class="form-check-label" for="sig">' . __d('messenger', 'Afficher la signature') . '</label>
                    </div>
                    <small class="help-block">' . __d('messenger', 'Cela peut être retiré ou ajouté dans vos paramètres personnels') . '</small>';
                }
                ...
                echo '
                    <input class="btn btn-primary" type="submit" name="submitS" value="' . __d('messenger', 'Valider') . '" />&nbsp;';
        
                if ($reply)
                    echo '
                    <input class="btn btn-danger ms-2" type="submit" name="cancel" value="' . __d('messenger', 'Annuler la réponse') . '" />';
                else {
                    echo '
                    <input class="btn btn-danger ms-2" type="submit" name="cancel" value="' . __d('messenger', 'Annuler l\'envoi') . '" />';

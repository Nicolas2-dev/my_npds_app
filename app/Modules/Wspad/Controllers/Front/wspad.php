        ...
        while (list($page) = sql_fetch_row($result)) {
            ...
            while (list($modtime, $editedby, $ranq, $verrou) = sql_fetch_row($result2)) {
                ...
                <tr>
                    ...
                    <td class="small">' . date(translate("dateinternal"), $modtime + ((int) Config::get('npds.gmt') * 3600)) . '</td>';
...
function Page($page, $ranq)
{
    ...
    echo '
    <hr /><h3>' . wspad_trans("Document : ") . '</h3><h4>' . $page . '<span class="text-muted">&nbsp;[ ' . wspad_trans("révision") . ' : ' . $row['ranq'] . ' - ' . $row['editedby'] . ' / ' . date(translate("dateinternal"), $row['modtime'] + ((int) Config::get('npds.gmt') * 3600)) . ' ] </span> <span class="float-end"><img src="modules/' . $ModPath . '/images/ajax_waiting.gif" id="verrous" title="wspad locks" /></span></h4>

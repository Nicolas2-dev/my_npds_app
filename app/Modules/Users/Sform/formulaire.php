...
$m->add_checkbox('user_viewemail', __d('users', 'Autoriser les autres utilisateurs à voir mon Email'), "1", false, false);
...
$m->add_field('user_from', __d('users', 'Votre situation géographique'), StripSlashes($user_from), 'text', false, 100, '', '');
$m->add_extender('user_from', '', '<span class="help-block"><span class="float-end" id="countcar_user_from"></span></span>');

$m->add_field('user_occ', __d('users', 'Votre activité'), StripSlashes($user_occ), 'text', false, 100, '', '');
$m->add_extender('user_occ', '', '<span class="help-block"><span class="float-end" id="countcar_user_occ"></span></span>');
$m->add_field('user_intrest', __d('users', 'Vos centres d\'intérêt'), StripSlashes($user_intrest), 'text', false, 150, '', '');
$m->add_extender('user_intrest', '', '<span class="help-block"><span class="float-end" id="countcar_user_intrest"></span></span>');

$m->add_field('user_sig', __d('users', 'Signature'), StripSlashes($user_sig), 'textarea', false, 255, '7', '');
$m->add_extender('user_sig', '', '<span class="help-block">' . __d('users', '(255 characters max. Type your signature with HTML coding)') . '<span class="float-end" id="countcar_user_sig"></span></span>');

// --- MEMBER-PASS
if (Config::get('npds.memberpass')) {
    $m->add_field('pass', __d('users', 'Mot de passe'), '', 'password', true, 40, '', '');
    ...
    $m->add_field('vpass', __d('users', 'Entrez à nouveau votre mot de Passe'), '', 'password', true, 40, '', '');
}

// --- MEMBER-PASS

$m->add_checkbox('user_lnl', __d('users', 'S\'inscrire à la liste de diffusion du site'), "1", false, true);
...
// --- CHARTE du SITE
$m->add_checkbox('charte', '<a href="static.php?op=charte.html" target="_blank">' . __d('users', 'Vous devez accepter la charte d\'utilisation du site') . '</a>', "1", true, false);
// --- CHARTE du SITE
// --- CONSENTEMENT

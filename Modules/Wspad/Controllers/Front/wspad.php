...
function Liste_Page()
{
    global $ModPath, $ModStart, $ThisFile, $auteur, $groupe, $couleur;
    ...
    if ($groupe > 0) {
        ...
        $aff .= '<span class="badge bg-secondary me-2">' . $nb_pages . '</span>' . __d('wspad', 'Document(s) et révision(s) disponible(s) pour le groupe') . ' <span class="text-muted">' . aff_langue($gp['groupe_name']) . " [$groupe]</span></h3>";
    } else
    ...
    if ($nb_pages > 0) {
        ...
        while (list($page) = sql_fetch_row($result)) {
            ...
            while (list($modtime, $editedby, $ranq, $verrou) = sql_fetch_row($result2)) {
                ...
                $PopUp = JavaPopUp("modules.php?ModPath=$ModPath&amp;ModStart=preview&amp;pad=" . encrypt($page . "#wspad#" . $groupe . "#wspad#" . $ranq), "App_wspad", 500, 400);
                ...
                if (($auteur == $verrou) or ($verrou == '')) {
                    ...
                    $PopUp = JavaPopUp("modules.php?ModPath=$ModPath&amp;ModStart=export&amp;type=doc&amp;pad=" . encrypt($page . "#wspad#" . $groupe . "#wspad#" . $ranq), "App_wspad", 5, 5);
                    $aff .= '
                    ...
                    $aff .= '<i class="text-danger fs-5 me-2 bi bi-lock-fill"></i>' . __d('wspad', 'Verrouillé par : ') . userpopover($verrou, '40', 2) . '</td>';
...
{
    global $ModPath, $ModStart, $auteur, $groupe, $mess;
    ...
    echo '
    <hr /><h3>' . __d('wspad', 'Document : ') . '</h3><h4>' . $page . '<span class="text-muted">&nbsp;[ ' . __d('wspad', 'révision') . ' : ' . $row['ranq'] . ' - ' . $row['editedby'] . ' / ' . date(__d('wspad', 'dateinternal'), $row['modtime'] + ((int) Config::get('npds.gmt') * 3600)) . ' ] </span> <span class="float-end"><img src="modules/' . $ModPath . '/images/ajax_waiting.gif" id="verrous" title="wspad locks" /></span></h4>
    ...
    if ($edition)
        ...
        <div class="mb-3">
            ...
            <a class="btn btn-secondary" href="modules.php?ModPath=' . $ModPath . '&amp;ModStart=' . $ModStart . '&amp;member=' . $groupe . '" >' . __d('wspad', 'Abandonner') . '</a>
            <input type="hidden" name="page" value="' . $page . '" />
...
switch ($op) {
    ...
    case "conv_new":
        ...
        $result = sql_query("INSERT INTO queue VALUES (NULL, $cookie[0], '$auteur', '$page', '" . FixQuotes($row['content']) . "', '', now(), '','$date_debval','$date_finval','0')");
        break;
}

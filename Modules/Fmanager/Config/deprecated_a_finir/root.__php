...
return [
    ...
    // $App_fma permet d'inclure le files-manager dans le thème de App
    $App_fma = false;

    // $css_fma permet d'inclure la css d'un thème / Cette option n'a de sens que si $App_fma=false
    if (($App_fma === false) and (file_exists("themes/".Config::get('npds.Default_Theme')."/style/f-manager.css")))
        $css_fma = "themes/".Config::get('npds.Default_Theme')."/style/f-manager.css";
    else
        $css_fma = "themes/".Config::get('npds.Default_Theme')."/style/style.css";

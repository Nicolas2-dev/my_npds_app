    ...
    public static function display($data)
    {
        ...
        or (static::$obj->FieldView == "jpeg")){
            ...
            $url_modifier = Config::get('npds.tiny_mce') 
                ? '"#" onclick="javascript:parent.tinymce.activeEditor.selection.setContent(\'<img class="img-fluid" src="'. site_url('getfile?att_id='. $data['ibid'] .'&amp;apli=f-manager') .'" />\');"' 
                : '"#"';
        } else {

            $url_modifier = Config::get('npds.tiny_mce') 
                ? '"#" onclick="javascript:parent.tinymce.activeEditor.selection.setContent(\'<a href="'. site_url('getfile?att_id='. $data['ibid'] .'&amp;apli=f-manager') .'" target="_blank">' . static::$obj->FieldName . '</a>\');"' 

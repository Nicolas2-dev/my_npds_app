 ...
 * Undocumented class
 */
class ForumNewtopic extends FrontController
{
        ...
        if ($submitS) {
            ...
            if ($stop != 1) {
                ...
                */
                ...
                $sql = "INSERT INTO forumtopics (topic_title, topic_poster, current_poster, forum_id, topic_time, topic_notify) VALUES ('$subject', '" . $userdata['uid'] . "', '" . $userdata['uid'] . "', '$forum', '$time'";
                ...
                $sql = "INSERT INTO posts (topic_id, image, forum_id, poster_id, post_text, post_time, poster_ip, poster_dns) VALUES ('$topic_id', '$image_subject', '$forum', '" . $userdata['uid'] . "', '$message', '$time', '$poster_ip', '$hostname')";
                ...
                <div class="alert alert-danger lead" role="alert">
                    ...
                    ' . __d('forum', 'Vous devez choisir un titre et un message pour poster votre sujet.') . '
                </div>';
            }
        } else {
            include('header.php');
            ...
            echo '
            <p class="lead">
                <a href="forum.php" >' . __d('forum', 'Index du forum') . '</a>&nbsp;&raquo;&raquo;&nbsp;<a href="viewforum.php?forum=' . $forum . '">' . stripslashes($forum_name) . '</a>
            </p>
                <div class="card">
            ...
            echo '
              ...
              <h4 class="my-3"><img width="48" height="48" class=" rounded-circle me-3" src="' . $imgava . '" alt="" />' . __d('forum', 'Poster un nouveau sujet dans :') . ' ' . stripslashes($forum_name) . '<span class="text-muted">&nbsp;#' . $forum . '</span></h4>
                 <blockquote class="blockquote">' . __d('forum', 'A propos des messages publiés :') . '<br />';
            ...
            if ($forum_access == 1) {
                if (!isset($user)) {
                    ...
                    <fieldset>
                        <div class="mb-3 row">
                        <label class="control-label col-sm-2" for="username">' . __d('forum', 'Identifiant : ') . '</label>
                        <div class="col-sm-8 col-md-4">
                            <input class="form-control" type="text" id="username" name="username" placeholder="' . __d('forum', 'Identifiant') . '" required="required" />
                        </div>
                        </div>
                        <div class="mb-3 row">
                        <label class="control-label col-sm-2" for="password">' . __d('forum', 'Mot de passe : ') . '</label>
                        <div class="col-sm-8">
                            <input class="form-control" type="password" id="password" name="password" placeholder="' . __d('forum', 'Mot de passe') . '" required="required" />
                        </div>
                        </div>
                    </fieldset>';
                ...
                if (user_is_moderator($userdata[0], $userdata[2], $forum_access)) {
                    echo '<strong>' . __d('forum', 'Auteur') . ' :</strong>';
                    echo $userdata[1];
                    $allow_to_post = 1;
                }
            ...
            if ($allow_to_post) {
                ...
                if ($myrow['forum_type'] == 8) {
                    ...
                    echo ' 
                    <div class="mb-3 row">
                        <label class="form-label" for="subject">' . __d('forum', 'Sujet') . '</label>
                        <div class="col-sm-12">
                        <input class="form-control" type="text" id="subject" name="subject" placeholder="' . __d('forum', 'Sujet') . '" required="required" value="' . $subject . '" />
                        </div>
                    </div>';
                    ...
                    if ($allow_html == 1)
                        echo '<span class="text-success float-end mt-2" title="HTML ' . __d('forum', 'On') . '" data-bs-toggle="tooltip"><i class="fa fa-code fa-lg"></i></span>' . HTML_Add();
                    else
                        echo '<span class="text-danger float-end mt-2" title="HTML ' . __d('forum', 'Off') . '" data-bs-toggle="tooltip"><i class="fa fa-code fa-lg"></i></span>';
                ...
                <div class="mb-3 row">
                    ...
                    if ($user) {
                        ...
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="notify2">' . __d('forum', 'Prévenir par Email quand de nouvelles réponses sont postées') . '</label>
                        </div>';
                    }
                ...
                <div class="mb-3 row">
                    <div class="col-sm-12">
                        ...
                        <input class="btn btn-primary" type="submit" name="submitS" value="' . __d('forum', 'Valider') . '" accesskey="s" />
                        <input class="btn btn-danger" type="submit" name="cancel" value="' . __d('forum', 'Annuler la contribution') . '" />
                    </div>
                </div>';
                }
            }
